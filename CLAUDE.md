...
4. **캐시 복원 직후 자동 폴링하지 않음.** 앱을 열면 캐시와 백그라운드 동기화가 미리 받아둔 글을 즉시 표시하고 로딩(`N/total` pill)이 뜨지 않음 — "오래 닫아둔 뒤 켜자마자 로딩"을 없애려는 의도. 백그라운드가 받아둔 글은 콜드 스타트 시 `consumeNativeSyncResults`로 게시물 캐시에 병합돼 피드에 바로 보이고, "checked Xm ago" 라벨도 마지막 백그라운드/수동 폴링 시각으로 갱신됨. 추가 갱신은 백그라운드 동기화(자동) 또는 당겨서 새로고침·Refresh 버튼(수동)으로만.
5. 앱을 백그라운드에서 다시 포그라운드로 가져올 때(`visibilitychange`)도 폴링하지 않음 — 백그라운드 워커가 가져온 글만 끌어와 "N new" 버퍼로 노출하고 "checked Xm ago"를 갱신함.
